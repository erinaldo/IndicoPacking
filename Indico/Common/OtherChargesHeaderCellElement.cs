...
{
    class OtherChargesHeaderCellElement : GridHeaderCellElement
    {
        private RadTextBoxElement txtPrice = null;

        public RadGridView ParentGrid { get; set; }

        public OtherChargesHeaderCellElement(GridViewColumn col, GridRowElement row)
            : base(col, row)
        {
            OtherChargesColumn column = (OtherChargesColumn)col;
        ...
        void btnApply_Click(object sender, EventArgs e)
        {
            ...
            while(grdRowEnumerator.MoveNext())
            {
                ...
                {
                    rowInfo.Cells["Other Charges"].Value = this.txtPrice.Text;
                }
            }
