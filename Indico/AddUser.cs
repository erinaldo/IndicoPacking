...
namespace IndicoPacking
{
    public partial class AddUser : Form
    {
        ...
        private void frmAddUser_Load(object sender, EventArgs e)
        {
            ...
            if(LoginInfo.Role == (int)LoggedUser.APP_ADMIN)
                this.cmbRole.DataSource = context.Roles.Where(u => u.ID != 1).ToList();
            else if (LoginInfo.Role == (int)LoggedUser.JK_ADMIN)
                this.cmbRole.DataSource = context.Roles.Where(u => u.ID == (int)LoggedUser.JK_ADMIN || u.ID == (int)LoggedUser.FILLING_CORDINATOR).ToList();
            else
                this.cmbRole.DataSource = context.Roles.Where(u => u.ID == (int)LoggedUser.INDIMAN_ADMIN).ToList();
